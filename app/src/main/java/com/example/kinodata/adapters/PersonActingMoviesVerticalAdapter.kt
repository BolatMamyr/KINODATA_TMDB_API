...
class PersonActingMoviesVerticalAdapter
    : RecyclerView.Adapter<PersonActingMoviesVerticalAdapter.MyViewHolder>() {

        var onItemClick: ((PersonActingMovies?) -> Unit)? = null
        private var movies = emptyList<PersonActingMovies>()
    ...
    fun updateData(newList: List<PersonActingMovies>) {
        val oldList = movies
    ...
    private class PersonMoviesDiffCallback(
        private val oldList: List<PersonActingMovies>,
        private val newList: List<PersonActingMovies>,
    ) : DiffUtil.Callback() {
