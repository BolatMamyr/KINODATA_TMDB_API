...
@AndroidEntryPoint
class RateMovieFragment : BottomSheetDialogFragment() {
    ...
    private fun deleteRating() {
        binding.btnBsDeleteRating.setOnClickListener {
    ...
    private fun observeRatingByUser() {
        viewModel.ratingByUser.observe(viewLifecycleOwner) { rating ->
            if (rating > 0) {
                ...
                ratingByUser = rating
                binding.btnBsDeleteRating.visibility = View.VISIBLE
            } else {
                binding.btnBsDeleteRating.visibility = View.GONE
            }
        }
    }
